                ...
                if (typeof param === 'object') {
                    if ('MutableReference' in param) {
                        if (typeof param.MutableReference === "object" && 'Struct' in param.MutableReference) {
                            ...
                            if (name !== "TxContext") {
                                dispatch({ type: MoveCallActionType.ADD_ARG, payload: `${address}::${module}::${name}` });
                            }
                        }
    ...
    const handleCall = async () => {
        try {
            ...
            if (executionStatus?.status === "failure") {
                ...
                dispatch({ type: MoveCallActionType.SET_RESPONSE, payload: JSON.stringify(response.digest) });
            }
            return response;
        } catch (err: any) {
            dispatch({ type: MoveCallActionType.SET_ERROR, payload: err.message });
        ...
        <>
            ...
            <div>
                {modulesName.length > 0 && <>
                    <span>Module: </span>
                    <select name="" id="" value={currentModule} onChange=
                        {handleChooseModule}>
            ...
            <div>
                ...
                {currentModule.length > 0 && <>
                    <span>Function: </span>
                    <select name="" id="" value={currentFunction} onChange=
            ...
            {currentFunction.length > 0 && <>
                ...
                {args.map((arg, index) => {
                    return <div key={arg}>
                        <Input placeholder={arg} value={argsUserInput[index] ? argsUserInput[index] : ""} onChange={(e) => handleSetValueToArg(index, e.target.value)} />
